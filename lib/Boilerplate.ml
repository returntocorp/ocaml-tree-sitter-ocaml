...
let map_capitalized_identifier (env : env) (tok : CST.capitalized_identifier) =
  token env tok (* pattern "[A-Z][a-zA-Z0-9_']*" *)

let map_conversion_specification (env : env) (tok : CST.conversion_specification) =
  token env tok (* conversion_specification *)
...
let map_indexing_operator (env : env) (tok : CST.indexing_operator) =
  token env tok (* indexing_operator *)

let map_match_operator (env : env) (tok : CST.match_operator) =
  token env tok (* match_operator *)
...
let map_concat_operator (env : env) (tok : CST.concat_operator) =
  token env tok (* concat_operator *)

let map_hash_operator (env : env) (tok : CST.hash_operator) =
  token env tok (* hash_operator *)
...
let map_right_quoted_string_delimiter (env : env) (tok : CST.right_quoted_string_delimiter) =
  token env tok (* right_quoted_string_delimiter *)
...
let map_prefix_operator (env : env) (tok : CST.prefix_operator) =
  token env tok (* prefix_operator *)

let map_pat_9465c8b (env : env) (tok : CST.pat_9465c8b) =
  token env tok (* pattern \\\n[\t ]* *)

let map_add_operator (env : env) (tok : CST.add_operator) =
  token env tok (* add_operator *)
...
let map_pat_21333c0 (env : env) (tok : CST.pat_21333c0) =
  token env tok (* pattern \\o[0-3][0-7][0-7] *)
...
let map_left_quoted_string_delimiter (env : env) (tok : CST.left_quoted_string_delimiter) =
  token env tok (* left_quoted_string_delimiter *)
...
let map_quoted_string_content (env : env) (xs : CST.quoted_string_content) =
  List.map (fun x ->
    ...
    | `LBRACKAT tok -> token env tok (* "[@" *)
    | `LBRACKATAT tok -> token env tok (* "[@@" *)
    | `LBRACKATATAT tok -> token env tok (* "[@@@" *)
    | `Pat_714c625 tok ->
        token env tok (* pattern [^%@|]+|%|@|\| *)
...
let map_label (env : env) ((v1, v2) : CST.label) =
  ...
  let v2 =
    token env v2 (* pattern "[a-z_][a-zA-Z0-9_']*" *)
  in
  todo env (v1, v2)
...
let map_abstract_type (env : env) ((v1, v2) : CST.abstract_type) =
  ...
  let v2 =
    List.map (token env) (* pattern "[a-z_][a-zA-Z0-9_']*" *) v2
  in
  todo env (v1, v2)
...
let map_class_path (env : env) (x : CST.class_path) =
  ...
  | `Module_path_DOT_id (v1, v2, v3) ->
      ...
      let v2 = token env v2 (* "." *) in
      let v3 =
        token env v3 (* pattern "[a-z_][a-zA-Z0-9_']*" *)
      in
      todo env (v1, v2, v3)
  )
...
let map_parenthesized_operator (env : env) ((v1, v2, v3) : CST.parenthesized_operator) =
  ...
  let v2 =
    ...
    | `Match_op tok -> token env tok (* match_operator *)
    )
  in
  let v3 = token env v3 (* ")" *) in
  todo env (v1, v2, v3)
...
and map_anon_choice_meth_type_345b567 (env : env) (x : CST.anon_choice_meth_type_345b567) =
  ...
  | `Meth_type (v1, v2) ->
      let v1 =
        token env v1 (* pattern "[a-z_][a-zA-Z0-9_']*" *)
      in
...
and map_class_field (env : env) (x : CST.class_field) =
  ...
  | `Inst_var_defi (v1, v2, v3, v4, v5, v6, v7, v8) ->
      ...
      let v4 =
        token env v4 (* pattern "[a-z_][a-zA-Z0-9_']*" *)
      in
      let v5 =
        (match v5 with
        | Some x -> map_typed env x
        | None -> todo env ())
      in
      let v6 =
        (match v6 with
...
and map_expression (env : env) (x : CST.expression) =
  ...
  | `For_exp (v1, v2, v3, v4, v5, v6, v7, v8) ->
      ...
      let v6 =
        ...
        | `Downto tok -> token env tok (* "downto" *)
        )
      in
...
and map_labeled_argument (env : env) (x : CST.labeled_argument) =
  ...
  | `Label_COLON_choice_simple_exp (v1, v2, v3) ->
...
and map_parameter_ (env : env) (x : CST.parameter_) =
  ...
  | `Label_COLON_simple_pat_ext (v1, v2, v3) ->
      let v1 = map_label env v1 in
      let v2 = token env v2 (* ":" *) in
      let v3 = map_simple_pattern_ext env v3 in
      todo env (v1, v2, v3)
  | `Choice_TILDE_LPAR_id_opt_typed_opt_EQ_seq_exp_ext_RPAR (v1, v2, v3, v4, v5, v6) ->
      let v1 = map_anon_choice_TILDE_72781e5 env v1 in
      let v2 = token env v2 (* "(" *) in
      let v3 =
        token env v3 (* pattern "[a-z_][a-zA-Z0-9_']*" *)
      ...
      let v6 = token env v6 (* ")" *) in
      todo env (v1, v2, v3, v4, v5, v6)
  | `Label_COLON_LPAR_pat_ext_opt_typed_EQ_seq_exp_ext_RPAR (v1, v2, v3, v4, v5, v6, v7, v8) ->
      let v1 = map_label env v1 in
      let v2 = token env v2 (* ":" *) in
      let v3 = token env v3 (* "(" *) in
...
and map_simple_expression (env : env) (x : CST.simple_expression) =
  ...
  | `Ocam_value tok -> token env tok (* pattern \$[0-9]+ *)
  )
...
and map_simple_pattern (env : env) (x : CST.simple_pattern) =
  (match x with
  | `Value_pat x -> map_value_pattern env x
  | `Signed_cst x -> map_signed_constant env x
...
and map_simple_type (env : env) (x : CST.simple_type) =
  ...
  | `Poly_vari_type v1 ->
      ...
      | `LBRACK_tag_spec_RBRACK (v1, v2, v3) ->
          ...
          let v3 = token env v3 (* "]" *) in
          todo env (v1, v2, v3)
      | `LBRACK_opt_tag_spec_BAR_tag_spec_rep_BAR_tag_spec_RBRACK (v1, v2, v3, v4, v5, v6) ->
          let v1 = token env v1 (* "[" *) in
          let v2 =
            (match v2 with
            | Some x -> map_tag_spec env x
            | None -> todo env ())
          in
          let v3 = token env v3 (* "|" *) in
      ...
      | `LBRACKGT_opt_BAR_opt_tag_spec_rep_BAR_tag_spec_RBRACK (v1, v2, v3, v4) ->
          ...
          let v4 = token env v4 (* "]" *) in
          todo env (v1, v2, v3, v4)
      | `LBRACKLT_opt_BAR_tag_spec_rep_BAR_tag_spec_opt_GT_rep1_tag_RBRACK (v1, v2, v3, v4, v5, v6) ->
          let v1 = token env v1 (* "[<" *) in
          let v2 =
            (match v2 with
            | Some tok -> token env tok (* "|" *)
  ...
  | `Obj_type (v1, v2, v3) ->
      ...
      let v2 =
        ...
        | Some x ->
            (match x with
            | `Choice_meth_type_rep_SEMI_choice_meth_type_opt_SEMI_opt_DOTDOT (v1, v2, v3) ->
...
and map_structure (env : env) (x : CST.structure) =
  (match x with
  | `Rep1_SEMISEMI xs -> List.map (token env) (* ";;" *) xs
  | `Rep_SEMISEMI_choice_stru_item_ext_rep_choice_rep_SEMISEMI_choice_stru_item_ext_rep_SEMISEMI (v1, v2, v3, v4) ->
      let v1 = List.map (token env) (* ";;" *) v1 in
      let v2 =
        (match v2 with
...
and map_type_binding (env : env) ((v1, v2, v3) : CST.type_binding) =
  ...
  let v2 =
    (match v2 with
    | `Id_opt_type_equa_opt_EQ_opt_priv_choice_vari_decl_rep_type_cons (v1, v2, v3, v4) ->
        let v1 =
          token env v1 (* pattern "[a-z_][a-zA-Z0-9_']*" *)
        in
        let v2 =
